...
FLAGS:
    ...
    -p  --print             Create nym-mixnode1.service for systemd
    -l  --print-local       Create nym-mixnode1.service for systemd LOCALLY in the current directory
...
function nym_download() {
 ...
 then
    printf "%b\n\n\n" "${WHITE} --------------------------------------------------------------------------------"
    printf "%b\n\n\n" "${YELLOW} Downloading ${WHITE} nym-mixnode1 binaries for the nym1 user ..."
    cd /home/nym1 && curl -LO https://github.com/nymtech/nym/releases/download/v0.9.2/nym-mixnode_linux_x86_64
    printf "%b\n\n\n"
    printf "%b\n\n\n" "${WHITE} nym-mixnode1 binaries ${LGREEN} successfully downloaded ${WHITE}!"
 else
    printf "%b\n\n\n"
...
function nym_chmod() {
 ...
 if ls -la /home/nym1/ | grep nym-mixnode_linux_x86_64 > /dev/null 2>&1
 then
   printf "%b\n\n\n" "${WHITE} --------------------------------------------------------------------------------"
   printf "%b\n\n\n" "${WHITE} Making the nym1 binary ${YELLOW} executable ..."
...
function nym_chown() {
 ...
 printf "%b\n\n\n" "${WHITE} --------------------------------------------------------------------------------"
 printf "%b\n\n\n" "${WHITE} Changed ownership of all conentes in ${YELLOW}/home/nym1/ ${WHITE} to ${YELLOW}nym1:nym1"
}
...
## It looks for multiple files in the /home/nym1/.nym/mixnodes directory
...
function nym_systemd_print() {
  printf "%b\n\n\n" "${WHITE} --------------------------------------------------------------------------------"
  printf "%b\n\n\n" "${YELLOW} Creating ${WHITE} a systemd service file to run nym-mixnode1 in the background: "
...
function nym_systemd_print_local() {
  directory='NymMixNode'
    ...
    current_path=$(pwd)
    if
      [ -e ${current_path}/nym-mixnode1.service ]
    then
...
function nym_systemd_run() {
    directory="NymMixNode"
    service_id=$(cat /etc/systemd/system/nym-mixnode1.service | grep id | cut -c 56-)
   ...
   ## Check if user chose a valid node written in the systemd.service file
    ...
    then
      ...
      printf "%b\n\n\n" "${WHITE} The node you selected is ${RED} not ${WHITE} in the  ${YELLOW} nym-mixnode1.service ${WHITE} file. Create a new systemd.service file with ${LBLUE} sudo ./nym-install.sh -p"
      exit 1
    fi
...
## Print the status nym-mixnode1.service
function nym_status() {
  systemctl status nym-mixnode1 | more
  if
      systemctl status nym-mixnode1 | grep -e "active (running)" > /dev/null 2>&1
    then
      printf "%b\n\n\n"
      ...
      systemctl status nym-mixnode1 | more | grep -i inactive  > /dev/null 2>&1
    then
      printf "%b\n\n\n"
...
## Full install, config and launch of the nym-mixnode
  if [ "$1" = "-i" ]; then
    while [ ! -d /home/nym1 ] ; do nym_usercreation ; done
    cd /home/nym1/ || printf "%b\n\n\n" "${WHITE}failed sorry"
    if [ ! -e /home/nym1/nym-mixnode_linux_x86_64 ] ; then nym_download ; fi
    nym_chmod
    nym_chown
    nym_init
    nym_systemd_print
...
## Configure the node
  if [[ ("$1" = "--init") ||  "$1" = "-c" ]]
  then
    cd /home/nym1/ > /dev/null 2>&1 && nym_init || printf "%b\n" "\n\n\n${YELLOW} /home/nym1/ ${RED} does not exist. ${WHITE} Create it with the ${YELLOW} -i ${WHITE} or ${YELLOW} --install ${WHITE} flag first.\n\n\n"
  fi
## Create the systemd.service file
  if [[ ("$1" = "--print") ||  "$1" = "-p" ]]
  then
    cd /home/nym1/  > /dev/null 2>&1 && nym_systemd_print || printf "%b\n" "\n\n\n${YELLOW} /home/nym1/ ${RED} does not exist. ${WHITE} Create it with the ${YELLOW} -i ${WHITE} or ${YELLOW} --install ${WHITE} flag first.\n\n\n"

  fi
##  Create the systemd.service file locally
  if [[ ("$1" = "--print-local") ||  "$1" = "-l" ]]
  then
  cd /home/nym1/  > /dev/null 2>&1 && nym_systemd_print_local || printf "%b\n" "\n\n\n${YELLOW} /home/nym1/ ${RED} does not exist. ${WHITE} Create it with the ${YELLOW} -i ${WHITE} or ${YELLOW} --install ${WHITE} flag first.\n\n\n"
    nym_systemd_print_local
  fi
## Run the node
  if [[ ("$1" = "--run") ||  "$1" = "-r" ]]
  then
    cd /home/nym1/.nym/mixnodes/ > /dev/null 2>&1  && nym_systemd_run || printf "%b\n" "\n\n\n${RED}no${YELLOW} config ${RED} found ${WHITE} Create it with the ${YELLOW} -c ${WHITE} or ${YELLOW} --init ${WHITE} flag first.\n\n\n"
  fi
